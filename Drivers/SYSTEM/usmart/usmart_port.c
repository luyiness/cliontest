 ...
 */
uint32_t usmart_timx_get_time(void)     //获取函数的执行时间，用执行前后的CNT值来算
{
...
{
    if(__HAL_TIM_GET_IT_SOURCE(&g_timx_usmart_handle,TIM_IT_UPDATE)==SET)/* 溢出中断，tclk=72MHz，故每100ms进入一次 */
    {
