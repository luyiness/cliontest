...
calendar_obj g_calendar;      //注意！！

calendar_obj sec2rtc_date(uint32_t seccount) {
    static uint16_t daycnt = 0;
    uint16_t temp1 = 0;
    uint32_t temp = seccount / 86400;    /* 得到天数(秒钟数对应的) */
    ...
    if (daycnt != temp)         /* 超过一天了 */
    {
        ...
        g_calendar.year = temp1;  /* 得到年份 */
        temp1 = 0;

        while (temp >= 28)      /* 超过了一个月 */
        {
            if (rtc_is_leap_year(g_calendar.year) && temp1 == 1) /* 当年是不是闰年/2月份 */
            {
                if (temp >= 29)
                {
                    temp -= 29; /* 闰年的秒钟数 */
        ...
        g_calendar.month = temp1 + 1; /* 得到月份 */
        g_calendar.date = temp + 1;   /* 得到日期 */
    }

    temp = seccount % 86400;                                                    /* 得到秒钟数 */
    g_calendar.hour = temp / 3600;                                                /* 小时 */
    g_calendar.min = (temp % 3600) / 60;                                          /* 分钟 */
    g_calendar.sec = (temp % 3600) % 60;                                          /* 秒钟 */
    g_calendar.week = rtc_get_week(g_calendar.year, g_calendar.month, g_calendar.date); /* 获取星期 */
    return g_calendar;
}